$(function(){
	...
	$.ajax({
		type:"get",
		url:"../data/goodDetail/goodDetail2/goodDetail2.json",
		success: function(data){
			...
			if(data.color){			
				...
				for(var key in data.color){
					var oDlColor = $('<dl class="clear"><dt class="fl>颜色</dt><dd class="color fl"></dd></dl>');
					$(".choice-buy .dl-num").before(oDlColor);
			...
			if(data.size){
				...
				for(var obj in data.size){
					var oDlSize = $('<dl class="clear dl-size"><dt class="fl">尺码</dt><dd class="size fl"><div class="size-wrap"></div><div class="box"><i class="error"></i>请选择尺码</div></dd></dl>');
					$(".choice-buy .dl-num").before(oDlSize);
